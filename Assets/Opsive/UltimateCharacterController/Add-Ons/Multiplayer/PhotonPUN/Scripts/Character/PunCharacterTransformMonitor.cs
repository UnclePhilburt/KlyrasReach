...
namespace Opsive.UltimateCharacterController.AddOns.Multiplayer.PhotonPun.Character
{
    ...
    public class PunCharacterTransformMonitor : MonoBehaviour, IPunObservable
    {
        ...
        private void Update()
        {
            ...
            if (m_InitialSync) {
                m_Transform.SetPositionAndRotation(m_NetworkPosition, m_NetworkRotation);
                m_InitialSync = false;
            }
            ...
            if (m_CharacterLocomotion != null && m_CharacterLocomotion.MovingPlatform != null) {
                ...
                m_NetworkPosition = MathUtility.TransformPoint(m_CharacterLocomotion.MovingPlatform.position, m_CharacterLocomotion.MovingPlatform.rotation, m_NetworkPlatformPrevRelativePosition);
                m_NetworkRotation = MathUtility.TransformQuaternion(m_CharacterLocomotion.MovingPlatform.rotation, m_NetworkPlatformPrevRotationOffset);
                m_CharacterLocomotion.SetPositionAndRotation(m_NetworkPosition, m_NetworkRotation, false, false);
            } else {
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting) {
                ...
                if (m_CharacterLocomotion != null && m_CharacterLocomotion.MovingPlatform != null) {
                    ...
                    var position = MathUtility.InverseTransformPoint(m_CharacterLocomotion.MovingPlatform.position, m_CharacterLocomotion.MovingPlatform.rotation, m_Transform.position);
                    var rotation = MathUtility.InverseTransformQuaternion(m_CharacterLocomotion.MovingPlatform.rotation, m_Transform.rotation);
                    if (position != m_NetworkPosition || m_InitialSync) {
                        dirtyFlag |= (byte)TransformDirtyFlags.Position;
                        m_NetworkPosition = position;
                    }
                    if (rotation != m_NetworkRotation || m_InitialSync) {
                    ...
                    // Determine the changed objects before sending them.
                    if (m_Transform.position != m_NetworkPosition || m_InitialSync) {
                        dirtyFlag |= (byte)TransformDirtyFlags.Position;
                    }
                    if (m_Transform.rotation != m_NetworkRotation || m_InitialSync) {
                        dirtyFlag |= (byte)TransformDirtyFlags.Rotation;
                    }
                ...
                if ((dirtyFlag & (byte)TransformDirtyFlags.Platform) != 0) {
                    ...
                    if (platformPhotonViewID != m_PlatformPhotonViewID) {
                        ...
                        m_CharacterLocomotion.SetMovingPlatform(platform, true);
                        m_NetworkPlatformRelativePosition = m_NetworkPlatformPrevRelativePosition = MathUtility.InverseTransformPoint(platform.position, platform.rotation, m_Transform.position);
                    ...
                    if ((dirtyFlag & (byte)TransformDirtyFlags.Position) != 0) {
                        ...
                        if (!m_InitialSync && !platformChange) {
                            // Account for the lag.
